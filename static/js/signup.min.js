...
return ['$scope', '$location', 'mnemonics', 'tx_sender', 'notices', 'wallets', '$window', 'facebook', '$modal', 'gaEvent', '$q', 'reddit', 'storage', 'trezor', 'btchip', 'bip38', '$interval', 'hw_detector',
        function SignupController($scope, $location, mnemonics, tx_sender, notices, wallets, $window, facebook, $modal, gaEvent, $q, reddit, storage, trezor, btchip, bip38, $interval, hw_detector) {
    ...
    $scope.signup.usbmodal = function() {
        ...
        hw_detector.waitForHwWallet().then(function() {
            var setup_d = $q.defer(), btchip = false;
            trezor.getDevice(true).then(function() {
                setup_d.resolve(trezor.setupSeed($scope.wallet.mnemonic));
            }, function() {
                btchip = true;
                setup_d.resolve(btchip.setupSeed($scope.wallet.mnemonic));
            });
            return setup_d.promise.then(function(result) {
                if (btchip) {
    ...
    $scope.signup.usb_hwseed_modal = function() {
        ...
        hw_detector.waitForHwWallet().then(function() {
            var setup_d = $q.defer();
            trezor.getDevice(true).then(function() {
                setup_d.resolve(trezor.setupSeed());
            }, function() {
                setup_d.resolve(btchip.setupSeed());
            });
            return setup_d.promise.then(function(result) {
